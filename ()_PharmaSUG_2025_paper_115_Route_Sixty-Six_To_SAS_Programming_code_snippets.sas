...
/***************************************************************************************************\
                                   ...
                                   WUSS 2024, Paper 125
                                   PharmaSUG 2025, Paper 115 
                                                                                                       
       _____             _           _____ _      _            _____ _         _                   
      |  __ \           | |         / ____(_)    | |          / ____(_)       | |                  
      | |__) |___  _   _| |_ ___   | (___  ___  _| |_ _   _ _| (___  ___  __  | |_ ___             
      |  _  // _ \| | | | __/ _ \   \___ \| \ \/ / __| | | |__\___ \| \ \/ /  | __/ _ \            
      | | \ \ (_) | |_| | ||  __/   ____) | |>  <| |_| |_| |  ____) | |>  <   | || (_) |           
      |_|  \_\___/ \__,_|\__\___|  |_____/|_/_/\_\\__|\__, | |_____/|_/_/\_\   \__\___/            
                                                       __/ |                                       
                                                      |___/                                        
   _____          _____                                                       _                      
  / ____|  /\    / ____|                                                     (_)                     
 ...
 |_____/_/    \_\_____/  | .__/|_|  \___/ \__, |_|  \__,_|_| |_| |_|_| |_| |_|_|_| |_|\__, |         
      ...
      PharmaSUG 2025 Conference, San Diego, California                                                                                               
...
options set=MAS_M2PATH="X:\SAShome\SASFoundation\9.4\tkmas\sasmisc\mas2py.py";
options set=MAS_PYPATH="C:\Users\bart\Python\Python312\python.exe";

/* Execute in command line:
C:
cd C:\Users\bart\Python\Python312
python -m pip install pandas saspy
*/

/* Setup SASPY.PY config file, located for example in:

C:\Users\bart\Python\Python312\Lib\site-packages\saspy\sascfg.py

For local Windows:

default  = {'saspath'  : 'X:/SASHome/SASFoundation/9.4/',
            'encoding': 'utf8',
            'java' : 'X:\\SASHome\\SASPrivateJavaRuntimeEnvironment\\9.4\\jre\\bin\\java'
           }
*/
...
cas CASAUTO sessopts=(caslib=casuser timeout=86400);
...
/*1 2 3 6 13 17 42 101 303 555 9999*/
proc sql noprint;
  select ids 
  into :ids separated by " "
  from caswork.SMALL;
...
run;
/*ERROR: File CASWORK.RESULT39.DATA does not exist.*/

/* !!! first load, because SAVE creates disc file */
proc casutil;
...
proc casutil;
  load data=WORK.BIG(WHERE=(id in (1 2 3 6 13 17 42 101 303 555 9999))) 
  outcaslib="casuser" casout="RESULT40" replace
  ;
run;
proc print data=caswork.RESULT40;
run;
...
/* fail
proc casutil;
  save casdata="BIG" replace;
run;
proc casutil;
  load incaslib='casuser' file="BIG.sashdat" outcaslib='casuser' casout='RESULT40' replace
  WHERE="id in (1 2 3 6 13 17 42 101 303 555 9999)"
  ;
run;
*/
...
https://documentation.sas.com/doc/en/pgmsascdc/9.4_3.5/caspg/cas-datastep-TblOfActions.htm
*/

proc cas;
  session casauto;

  c.BIG    = "casuser.BIG(in=B)";
...
libname caswork list; 
proc print data=caswork.RESULT43;
run;
...
/*
FedSQL Action Set:
https://documentation.sas.com/doc/en/pgmsascdc/9.4_3.5/caspg/cas-fedsql-TblOfActions.htm

Join Tables with fedSql.execDirect:
https://documentation.sas.com/doc/en/pgmsascdc/9.4_3.5/caspg/p0aeici8mm7ivnn1pc24uywi6rn7.htm
*/
proc cas; 
  session casauto;  
  ...
  table.fetch / 
    table="RESULT44"
    sortby={{name="id", order="descending"}}
    to=&sysmaxlong.,
    index=FALSE
  ;
  run;
...
libname caswork list; 
proc print data=caswork.RESULT44;
run;




/* Statements and user defined functionas */
proc cas; 
  session casauto;  
  ...
  function tableLookUp(BIG,small);
    ...
    table.fetch result=B / /* fetch data */
      table=BIG,
      maxRows=Bnr,
      from=1,
      to=Bnr,
    ...
    table.fetch result=RESULT45 / /* get a "template" */
      table=BIG,
      maxRows=0,
      index=FALSE;
...
libname caswork list; 
proc print data=caswork.RESULT45;
run;
...
options obs=1;
data list;
  set WORK.RESULT: S.RESULT: CASWORK.RESULT: indsname=i;
  indsname=i;
  keep indsname;
run;
options obs=MAX;
